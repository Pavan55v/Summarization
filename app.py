...
def get_summary():
	...
	label_size_main.config(text=f'Original size: {num_words(raw_text)} Words ')
	final_text = text_summarizer(raw_text)
	print(final_text)
	summarized_label_size_main.config(text=f'Summarized size: {num_words(final_text)} Words')
...
def openfiles():
	...
	read_text = open(file1,'rb').read()
	label_size_file.config(text=f'size: {num_words(read_text)} words')
...
def get_file_summary():
	...
	final_text = text_summarizer(raw_text)
	summarized_label_size_file.config(text=f'summarized size: {num_words(final_text)} words')
	result = '\nSummary:{}'.format(final_text)
...
def get_text():
	...
	label_size_url.config(text=f'size: {num_words(fetched_text)} words ')
...
def get_url_summary():
	...
	final_text = text_summarizer(raw_text)
	result = '\nSummary:{}'.format(final_text)
	summarized_label_size_url.config(text=f'summarized size: {num_words(final_text)} words')
...
label_size_main = Label(tab1, text="Original Size : 0")
label_size_main.grid(row=6, column=0)

summarized_label_size_main = Label(tab1, text="Summarized Size : 0")
summarized_label_size_main.grid(row=6, column=1)
...
label_size_file=Label(tab2,text="Original Size : 0")
label_size_file.grid(row=6,column=1)

summarized_label_size_file=Label(tab2,text="Summarized Size : 0")
summarized_label_size_file.grid(row=6,column=2)
...
label_size_url=Label(tab3,text="Original Size : 0")
label_size_url.grid(row=6,column=1)

summarized_label_size_url=Label(tab3,text="Summarized Size : 0")
summarized_label_size_url.grid(row=6,column=2)
# Display Screen For Result
...
# About TAB
about_label = Label(tab5,text="Text Summarizer\n Used to summarize a long text into short one",pady=5,padx=5)
